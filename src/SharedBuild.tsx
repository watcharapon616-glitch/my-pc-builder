...
// จำลองข้อมูลเซ็ตคอมพิวเตอร์มาตรฐาน 
const PREBUILT_SETS = [
  { id: 's1', name: 'EXTREME GODZILLA', cpu: 'i9-14900K', gpu: 'RTX 4090', price: 155000, score: 99 },
  { id: 's2', name: 'GAMING BEAST', cpu: 'Ryzen 7 7800X3D', gpu: 'RTX 4080 Super', price: 89000, score: 92 },
  { id: 's3', name: 'PRO STREAMER', cpu: 'i7-14700K', gpu: 'RTX 4070 Ti', price: 65000, score: 85 },
  { id: 's4', name: 'VALUE KING', cpu: 'i5-13400F', gpu: 'RTX 4060', price: 28000, score: 65 },
  { id: 's5', name: 'BUDGET FIGHTER', cpu: 'Ryzen 5 5600', gpu: 'RX 6600', price: 18500, score: 55 },
  { id: 's6', name: 'WORKSTATION LITE', cpu: 'i5-14400', gpu: 'RTX 3060', price: 32000, score: 70 },
];
...
export default function SharedBuild() {
  ...
  // 1. อันดับความเร็วสูงสุด (เรียงตาม Score)
  const topSpeedSets = [...PREBUILT_SETS].sort((a, b) => b.score - a.score).slice(0, 3);
  
  // 2. อันดับความคุ้มค่าสูงสุด (เรียงตาม Score ต่อ Price)
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#020617] p-4 md:p-10 transition-colors">
      <div className="max-w-6xl mx-auto">
        <button onClick={() => navigate('/build')} className="mb-6 flex items-center gap-2 text-sm font-black dark:text-cyan-400 uppercase tracking-widest">
          <ArrowLeft size={16} /> กลับไปหน้าจัดสเปค
        </button>

        {/* ส่วนแสดงคะแนนเครื่องปัจจุบัน */}
        <div className="mb-12 bg-white dark:bg-slate-900 rounded-[3rem] p-8 border-4 border-slate-900 dark:border-white/10 shadow-xl flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-6">
             ...
             <div>
                ...
                <h2 className="text-3xl font-black dark:text-white italic uppercase tracking-tighter">สเปคที่คุณจัดเอง</h2>
                <p className="font-mono text-slate-400">Total: {data.totalPrice.toLocaleString()} ฿</p>
             </div>
          </div>
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          {/* ส่วนที่ 1: 3 อันดับเซ็ตที่เร็วที่สุด */}
          ...
          {/* ส่วนที่ 2: 3 อันดับเซ็ตที่คุ้มค่าที่สุด */}
          <div className="space-y-6">
            <div className="flex items-center gap-3 mb-2">
                ...
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      ...
                      <span className="text-[9px] px-2 py-0.5 bg-green-500/10 text-green-500 rounded-md font-bold">คุ้มค่าที่สุด</span>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-[10px] font-black text-slate-400 uppercase">Perf/Price Ratio</p>
                    <p className="text-3xl font-black text-green-500 italic">{(set.score / set.price * 100000).toFixed(1)}</p>
                  </div>
                </div>
              </div>
            ))}
